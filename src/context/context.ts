...
export class Context {
  ...
  //check if the context object key is case insensitive
  ...
  public constructor(
    ...
    this.strictVariables =
      renderOptions.strictVariables ?? this.opts.strictVariables;
    this.ownPropertyOnly =
      renderOptions.ownPropertyOnly ?? opts.ownPropertyOnly;

    //added for case insensitive
    this.caseInsensitive =
      renderOptions.caseInsensitive ?? opts.caseInsensitive;
  }
  ...
  public *_getFromScope(
    ...
    for (let i = 0; i < paths.length; i++) {
      scope = yield readProperty(
        scope as object,
        paths[i],
        this.ownPropertyOnly
      );
      if (isNil(scope) && this.strictVariables) {
        throw new InternalUndefinedVariableError(
          (paths as string[]).slice(0, i + 1).join!(".")
        );
      }
    }
    return scope;
  ...
  /**
   *Returns the scope for the key
   *Case-insensitive part was added.
  ...
  */
  ...
  private findScope(key: string | number) {
    ...
    //in case of case-insensitive
    if (this.caseInsensitive === true && typeof key === "string") {
      //boolean variable to check if the key is the same if case insensitive
      let flag: Boolean = false;

      //convert the context object to the object with option key
      const environment_case_insensitive = Object.entries(this.environments)
        .map(([key_env, value]) => {
          if (key_env.toLocaleLowerCase() === key.toString().toLowerCase()) {
            flag = true;
            return [key, value];
          } else {
      ...
      if (flag === true) {
        return environment_case_insensitive;
      }
    }
