...
export class Liquid {
  ...
  public constructor(opts: LiquidOptions = {}) {
    ...
    forOwn(tags, (conf: TagClass, name: string) =>
      this.registerTag(name, conf)
    );
    forOwn(filters, (handler: FilterImplOptions, name: string) =>
      this.registerFilter(name, handler)
    );
  }
  ...
  public async render(
    ...
    return toPromise(
      this._render(tpl, scope, { ...renderOptions, sync: false })
    );
  }
  public renderSync(
    tpl: Template[],
    ...
    return toValueSync(
      this._render(tpl, scope, { ...renderOptions, sync: true })
    );
  }
  public renderToNodeStream(
    tpl: Template[],
    scope?: object,
    renderOptions: RenderOptions = {}
  ...
  public async parseAndRender(
    html: string,
    scope?: Context | object,
    renderOptions?: RenderOptions
  ): Promise<any> {
    return toPromise(
      this._parseAndRender(html, scope, { ...renderOptions, sync: false })
    );
  }
  public parseAndRenderSync(
    html: string,
    scope?: Context | object,
    renderOptions?: RenderOptions
  ): any {
    return toValueSync(
      this._parseAndRender(html, scope, { ...renderOptions, sync: true })
    );
  }
  ...
  public async parseFile(
    ...
    return toPromise<Template[]>(
      this.parser.parseFile(file, false, lookupType)
    );
  }
  public parseFileSync(file: string, lookupType?: LookupType): Template[] {
    return toValueSync<Template[]>(
      this.parser.parseFile(file, true, lookupType)
    );
  }
  public *_renderFile(
    file: string,
  ...
  public async renderFile(
    ...
    return toPromise(
      this._renderFile(file, ctx, { ...renderFileOptions, sync: false })
    );
  }
  public renderFileSync(
    file: string,
    ctx?: Context | object,
    renderFileOptions?: RenderFileOptions
  ) {
    return toValueSync(
      this._renderFile(file, ctx, { ...renderFileOptions, sync: true })
    );
  }
  public async renderFileToNodeStream(
    file: string,
    scope?: object,
    renderOptions?: RenderOptions
  ) {
  ...
  public _evalValue(
    ...
    const ctx =
      scope instanceof Context ? scope : new Context(scope, this.options);
  ...
  public express() {
    ...
    return function (
      ...
      self
        .renderFile(filePath, ctx)
        .then((html) => callback(null, html) as any, callback as any);
    };
  }
}
